...
node[:deploy].each do |application, deploy|
  ...
  settings[:newegg_crawlers].times do |idx|
    idx = idx + 1 # make index 1-based
    template "/etc/init/newegg-crawler-#{application}-#{idx}.conf" do
      source "site-crawler-n.conf.erb"
      mode '0644'
      variables application: application, site: 'newegg', rack_env: rack_env, deploy: deploy, instance: idx
    end
  end
end