...
node[:deploy].each do |application, deploy|
  ...
  settings[:toysrus_crawlers].times do |idx|
    idx = idx + 1 # make index 1-based
    template "/etc/init/toysrus-crawler-#{application}-#{idx}.conf" do
      source "site-crawler-n.conf.erb"
      mode '0644'
      variables application: application, site: 'toysrus', rack_env: rack_env, deploy: deploy, instance: idx
    end
  end
end