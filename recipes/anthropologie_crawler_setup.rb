...
node[:deploy].each do |application, deploy|
  ...
  settings[:anthropologie_crawlers].times do |idx|
    idx = idx + 1 # make index 1-based
    template "/etc/init/anthropologie-crawler-#{application}-#{idx}.conf" do
      source "site-crawler-n.conf.erb"
      mode '0644'
      variables application: application, site: 'anthropologie', rack_env: rack_env, deploy: deploy, instance: idx
    end
  end
end